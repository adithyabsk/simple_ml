...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  // BEGIN YOUR SOLUTION
  // this is not set to size in the case of
  // offset, size is just the full size of the output array
  size_t shape_prod = 1;
  for(size_t i=0; i <shape.size; i++) {
    shape_prod*=shape.data[i];
  }
  ...
  if (gid < shape_prod) {
    ...
    for(size_t j=0; j<shape.size; j++){
      prod /= shape.data[j];
      a_idx+=(remainder/prod)*strides.data[j];
      remainder %= prod;
    }
    out[gid] = a[a_idx];
  }
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, std::vector<int32_t> shape,
  ...
  PrintVector(shape);
  PrintVector(strides);
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(out->ptr, val, out->size, VecToCuda(shape),
                                         VecToCuda(strides), offset);
 /// END YOUR SOLUTION
}
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b,
                             scalar_t* out, size_t size, uint32_t M, uint32_t N,
                             uint32_t P) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
